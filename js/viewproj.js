	...
	if (actAddItem.val() !== "" || actAddItemQty.val() !== "") {
		if (ActModalMode == "add") {
			...
			for (var i = 0; i < AddActItems.length; i++) {
				if (AddActItems[i]["resourceid"] == selected && AddActItems[i]["type"] == "0") {
					existing = true;
			...
			for (var i = 0; i < theAct["items"].length; i++) {
				if (theAct["items"][i]["ResourceID"] == selected && theAct["items"][i]["Type"] == "0") {
					existing = true;
			...
			for (var i = 0; i < EditOperations["add"].length; i++) {
				if (EditOperations["add"][i]["resourceid"] == selected && EditOperations["add"][i]["type"] == "0") {
					existing = true;
